...
def find_cost(X, y, W , reg):
    ...
    # prediction
    pred = X@W
    # l1 regularized penalty term
    penalty = sum(np.abs(W))
    # lasso cost function: (1/n) * (l2 norm (y-pred)^2 + reg*penalty_
    cost = (np.linalg.norm(y-pred)**2 + reg*penalty) / len(y) 
...
def find_grad(X, y, W, reg=0.0):
    ...
    "*** YOUR CODE HERE ***"
    # can't take the derivate of an L1 norm, so only do the loss 
    # function gradient here
    m = X.shape[0]
    grad = X.T@(X@W-y) / m
...
def prox(X, gamma):
    ...
    """*** YOUR CODE HERE ***"""
    # adjust the weight vector based on the given gamma
    # if a weight is small, then the gradient is weakly correlated, so our 
    # subgradient is 0
    X[np.abs(X)<=gamma]=0
    # if the gradient is negatively correlated (X < -gamma) then the subgradient
    # is w = w+gamma
    X[X < -gamma] +=gamma
    # if the gradient is positively correlated (X > gamma) then the subgradient
    # is w = w-gamma
    X[X > gamma] -= gamma
...
def grad_lasso(
    ...
    while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
        ...
        "*** YOUR CODE HERE ***"
        # randomly select rows of data to use for our training
        idxs = np.random.randint(0,m,batch_size)
        curX = X[idxs]
        curY = y[idxs]
        # for the following steps for the selected indices
        # find the gradient from the loss function
        W_grad = find_grad(curX,curY,W,reg=reg)
        # adjust W based on the gradient and subgradient (prox)
        # usual gradient update
        newW = W - lr*W_grad
        # calculate gamma for prox
        gamma = lr*reg
        # subgradient adjustment
        W = prox(newW,gamma)
        # find the error with our cost function
        cost = find_cost(curX,curY,W,reg=reg)
...
def lasso_path(X, y, tau_min=1e-8, tau_max=10, num_reg=10):
    ...
    for index in range(num_reg):
        ...
        #         as they are.
        """*** YOUR CODE HERE ***"""
        colW, errList = grad_lasso(X, y, reg=reg, batch_size=1024, print_freq=10000)
        W[:,index] = colW.flatten()
...
if __name__ == '__main__':
    ...
    # skip the url column
    feats = df.columns[1:]
    # sort the weights in the first column, based on the abs values
    idxs = np.argsort(np.abs(W[:,0]))
    sortedFeats = feats[idxs]
    top_features = sortedFeats[:5]
    """*** END YOUR CODE HERE ***"""
    print(top_features)
