...
def linreg(X, y, reg=0.0):
    ...
    I = np.eye(X.shape[1])
    # skip the bias term
    # for ridge regression, we penalize by the weights. However, it doesn't 
    # make sense to penalize for having a large constant offset. So we set
    # that value to be 0, so it is ignored in the fitting process.
    I[0,0]=0
    # use @ for matrix multiplying
    # use ridge regression equation
    W_opt = np.linalg.inv(reg*I + X.T@X)@X.T@y
...
def find_RMSE(W, X, y):
    ...
    "*** YOUR CODE HERE ***"
    y_hat = predict(W,X)
    sqErr = np.square(y-y_hat)
    normErr = sqErr/y.shape[0]
    # get out the value from the matrix
    RMSE = np.sqrt(sum(normErr))[0,0]
    "*** END YOUR CODE HERE ***"
    return RMSE
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
    ...
    # num Lambdas/reg
    size = 150
    # generate reg values aka lambda values (complxity penality)
    # sort for graphing purposes.
    reg_list = sorted(np.random.uniform(low=0, high=150, size = size))
    # compute optimal w values for the train set for each given penality value
    W_list = [linreg(X_train,y_train, reg =i) for i in reg_list]
    # compute RMSE for each optimal W value on the validation set
    RMSE_list = [find_RMSE(i,X_val, y_val) for i in W_list]
    ...
    "*** YOUR CODE HERE ***"
    # find min of the RMSE err list, and use its index to find the optimal 
    # lambda aka reg value
    minIdx = np.argmin(RMSE_list)
    # optimal reg /lambda based on error
    reg_opt = reg_list[minIdx]
...
def norm_vs_lambda(X_train, y_train, X_val, y_val):
    ...
    "*** YOUR CODE HERE ***"
    size = 150
    # generate reg values aka lambda values (complexity penality)
    # sort for graphing purposes aka list of weights
    reg_list = sorted(np.random.uniform(low=0, high=150, size = size))
    # compute optimal w values for the train set for each given penality value
    W_list = [linreg(X_train,y_train, reg =i) for i in reg_list]
    # compute RMSE for each optimal W value on the validation set
    norm_list = [np.linalg.norm(i) for i in W_list]
...
def linreg_no_bias(X, y, reg=0.0):
    ...
    m,n = X.shape
    ones = np.ones(m)
    centering = (np.eye(m) - (ones/m))
    # gamma should be a nxn matrix.
    # gamma is a diagonal matrix with the regularization values
    # so gamma.T@gamma = np.eye(c)*reg
    gamma = np.eye(n)* reg
    inv = np.linalg.inv(X.T@centering@X + gamma)
    W_opt = inv@X.T@centering@y
    
    b_opt = np.dot(ones,(y-X@W_opt)) / m
...
def grad_descent(X_train, y_train, X_val, y_val, reg=0.0, lr_W=2.5e-12, \
    ...
    while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps \
        ...
        "*** YOUR CODE HERE ***"
        # use the following equation for error (see equation in part d)
        # err = (L2 norm (X.T@W + b-y))**2
        # we want to take into account the calculated bias and weight values 
        # that we find, so don't use RMSE function
        
        errTrain = np.sqrt(np.linalg.norm(X_train @ W + b - y_train) ** 2 / m_train)
        errTest = np.sqrt(np.linalg.norm(X_val @ W + b - y_val) ** 2 / m_val)
            
        # save out the err to the appropriate lists
        obj_train.append(errTrain)
        obj_val.append(errTest)
        # based on reg value create gamma
        gamma = np.eye(n)* reg
        # ones matrix
        ones = np.matrix(np.ones(m_train))
        # compute gradient based on the equations
        W_grad = (X_train.T@X_train@W + b*X_train.T.dot(ones.T) \
            - X_train.T@y_train + gamma@W)/m_train
...
if __name__ == '__main__':
    ...
    "*** YOUR CODE HERE ***"
    # add column ones to account for the constant offset.
    # add the column to the beginning such that the constant is the first column.
