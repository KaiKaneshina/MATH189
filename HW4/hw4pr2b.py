...
def NLL(X, y, W, reg=0.0):
    ...
    # calculate numerator
    expMu = np.exp(mu)
    # calculate denom, reshape so we are m x 1 
    denom = expMu.sum(axis=1).reshape(-1,1)
    # do probability function
    probability = expMu/denom
    # use the y as the indicator function: elementwise multiplication of y 
    # with the log of the probability will only keep the relevant values of 
    # probability matrix
    preSum = y*np.log(probability)
    # sum over the number of categories
    sum1 = preSum.sum(axis=1)
    # sum again over the number of features (m)
    total = sum1.sum()
    # multiply by -1 to get NLL , then add in the regularization matrix
    NLL = -1*total + reg*(np.linalg.norm(W)**2)
...
def predict(X, W):
    ...
    # do probability function
    probability = expMu/denom
    # prediction is the category with the high probability, so return an array 
    # of shape m (number of data points), with the prediction as the category
...
def accuracy_vs_lambda(X_train, y_train_OH, X_test, y_test, lambda_list):
    ...
    for reg in lambda_list:
        ...
        "*** YOUR CODE HERE ***"
        # find optimal weight on training data for the given reg
        W, nll_list = grad_descent(X_train, y_train_OH,reg=reg)
        # predict on the test data
        preds = predict(X_test,W)
        # calculate accuracy based on the y_test truth values
    ...
    "*** YOUR CODE HERE ***"
    idx = np.argmax(np.array(accu_list))
    reg_opt = lambda_list[idx]
    "*** END YOUR CODE HERE ***"
...
if __name__ == '__main__':
    ...
    X_test = data.X_test
    y_test = data.y_test
    # df_train = pd.read_csv('/Users/kaikaneshina/Downloads/hw4_starter_files/df_train.csv')#data.df_train
    # df_test = pd.read_csv('/Users/kaikaneshina/Downloads/hw4_starter_files/df_test.csv')#data.df_test

    # # training data
    # X_train = np.load('/Users/kaikaneshina/Downloads/hw4_starter_files/X_train.npy') #data.X_train
    # y_train = np.load('/Users/kaikaneshina/Downloads/hw4_starter_files/y_train.npy') #data.y_train

    # # test data
    # X_test = np.load('/Users/kaikaneshina/Downloads/hw4_starter_files/X_test.npy')#data.X_test
    # y_test = np.load('/Users/kaikaneshina/Downloads/hw4_starter_files/y_test.npy') #data.y_test
