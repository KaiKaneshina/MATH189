...
def NLL(X, y, W, reg=0.0):
    ...
    "*** YOUR CODE HERE ***"
    penalty = reg* (np.linalg.norm(W)**2)
    sig = sigmoid(X@W)
    # remove sig values that are negative before we do the log.
    sig[sig<=0] = 1
    diff = 1-sig
    diff[diff<=0]=1
    likelihood = np.multiply(y,np.log(sig)) + np.multiply((1-y),(np.log(diff)))
    nll = -1*likelihood.sum() + penalty
    "*** END YOUR CODE HERE ***"
    return nll
...
def newton_step(X, y, W, reg=0.0):
    ...
    mu = sigmoid(X@W)
    # calculate S matrix
    S = np.diag(np.multiply(mu,(1-mu)).flatten())
    # calculate hessian
    H = X.T@S@X + reg*np.eye(X.shape[1])
    # solve for d
...
def newton_method(X, y, reg=0.0, eps=1e-6, max_iter=20, print_freq=5):
    ...
    while iter_num < max_iter and np.linalg.norm(step) > eps:
        ...
        nll = NLL(X, y, W,reg=reg)
        nll_list.append(nll)
        # calculate gradient with newton step
        step = newton_step(X, y, W,reg=reg)
        # use newtons method: x_1 = x0 - (f_prime/f_double_prime)
        W = W -step
...
def plot_description(X_train, y_train, X_test, y_test):
    ...
    "*** YOUR CODE HERE ***"
    reg_list =[0] + sorted(list(np.random.uniform(10,100,9)))
    for reg in reg_list:
         W, nll_list = grad_descent(X_train, y_train, reg = reg)
         accuracy, precision, recall, f1 = get_description(X_train,y_train,W)
    ...
    "*** YOUR CODE HERE ***"
    idx = np.argmax(np.array(a_list))
    reg_opt = reg_list[idx]
