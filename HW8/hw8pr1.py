...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
    ...
    while iter_num < max_iter:
        ...
        "*** YOUR CODE HERE ***"
        # save the previous clusters before updating them
        # need to do a comparison of cluster difference between iterations to see if 
        # we have converged enough
        prev_clusters = copy.deepcopy(clusters)
        
        # loop through all data points and compare each point to each cluster centroid, 
        # find the argmin and then set the label accordingly
        # for i in range(m):
        #     row = X[i,:]
        #     diff = row - clusters
        #     # diff is now k x n, want to do linalg norm for each row (k)
        #     label[i] = np.argmin(np.linalg.norm(diff, axis = 1)**2)
        
        ## faster: apply the argmin of the distance for each cluster along each 
        # row of X
        label = np.apply_along_axis(lambda x: np.argmin(np.linalg.norm(x-clusters, axis = 1)),1, X)

        # for each cluster/centroid, we need to recalculate it based on the labels
        for i in range(k):
            # this is our indicator function
            idxs = np.where(label==i)[0]
            # update each row of the clusters
            if len(idxs) > 0:
                clusters[i,:] = np.mean(X[idxs,:],axis = 0)
...
def k_means_cost(X, clusters, label):
    ...
    "*** YOUR CODE HERE ***"
    # labels are just the indices of which cluster to choose, so grab the appropriate clusters 
    # for data
    xClusters = clusters[label]
    cost = sum(np.linalg.norm(X-xClusters, axis = 1)**2)
...
if __name__ == '__main__':
    ...
    plt.plot(np.arange(1,21), cost_k_list,'g^')
    # plot optimal k vs optimal costs
    ...
    # grab the xCluster labels
    xClusters = clusters[label]
    # plot the original data
    data_plot, = plt.plot(X[:, 0], X[:, 1], 'bo')
    # plot the clustered data
    center_plot, = plt.plot(xClusters[:, 0], xClusters[:, 1],'rD')
    "*** END YOUR CODE HERE ***"
